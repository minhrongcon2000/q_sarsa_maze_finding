from agent.sarsa import SARSA
import gym
import gym_maze

env = gym.make("maze-sample-10x10-v0")
env = gym.wrappers.Monitor(env, "sarsa", force=True)
agent = SARSA(env=env, discounted_factor=0.9)
episodes = 10000
horizon = 10000
epsilon = 1
alpha = 1

for episode in range(episodes):
    observation = env.reset()
    for t in range(horizon):
        env.render()
        x, y = observation
        x, y = int(x), int(y)
        current_coord = (x, y)
        
        current_action = agent.optimal_policy[x, y]
        current_action = int(current_action)
        observation, reward, done, _ = env.step(action=current_action)
        
        x_next, y_next = observation
        x_next, y_next = int(x_next), int(y_next)
        next_coord = (x_next, y_next)
        
        if done:
            break
        
        agent.update_policy(
            current_coord=current_coord,
            current_action=current_action,
            current_reward=reward,
            next_coord=next_coord,
            next_action=int(agent.optimal_policy[x_next, y_next]),
            learning_rate=alpha,
            exploration_rate=epsilon
        )
    else:
        env.stats_recorder.save_complete()
        env.stats_recorder.done = True
    
    print("Episode {} completed".format(episode + 1))
    print(agent.Q_table)
    epsilon = 1 / (episode + 1)
    alpha = 1 / (episode + 1)
env.close()
